    ...
    def lookup(
        ...
        try:
            columns = '''
                ...
                FROM entries e
            '''
            query = columns + ' WHERE e.normalized_headword = ?'
            params: list[object] = [lemma]
            if part_of_speech:
                query += ' AND (e.part_of_speech = ? OR e.part_of_speech IS NULL)'
                params.append(part_of_speech)
            ordering = ' ORDER BY e.frequency_rank IS NULL, e.frequency_rank, e.id LIMIT ?'
            query += ordering
            params.append(limit)
            rows = connection.execute(query, params).fetchall()
            if not rows:
                query = columns + ' JOIN aliases a ON a.entry_id = e.id'
                query += ' WHERE a.normalized_form = ?'
                params = [lemma]
                if part_of_speech:
                    query += ' AND (e.part_of_speech = ? OR e.part_of_speech IS NULL)'
                    params.append(part_of_speech)

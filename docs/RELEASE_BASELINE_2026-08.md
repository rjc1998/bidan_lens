...
`70fec242d3ee7f04f743fbb17f8494c98a1c703d16b99ca6397c23ff6efc5943` and
...
The following small-text OCR batch uses a production-recognizer-only binarized retry for
two-to-five-syllable pure-Hangul words below 99.8% confidence on detector lines no taller than
14.1 px. It autocontrasts and thresholds 3x crops at 216, then requires bilinear, bicubic, and
Lanczos recognition to agree on one same-length alternative. The conservative minimum retry
confidence must reach 94% and exceed the original. A broader 16 px experiment without the 94%
floor was rejected because it added `dev-plain-0538`, `dev-plain-1729`, and
`dev-plain_stress-0157` and lowered the 14 px stratum.

The accepted exact comparison removes five main context diagnostics
(`dev-plain-0001`, `dev-plain-0801`, `dev-plain-1281`, `dev-plain-1601`, and
`dev-plain-1889`) and three stress context diagnostics (`dev-plain_stress-0089`,
`dev-plain_stress-0117`, and `dev-plain_stress-0185`), with no addition or changed record.
The 12 px stratum rises from 98.10% to 98.27% OCR, 84.00% to 86.00% context, 61.60% to
63.20% exact transcription, and 80.80% to 82.80% popup correctness. Bracket and
natural-punctuation strata improve. Stress OCR, context, exact transcription, and popup
correctness rise to 94.36%, 68.40%, 50.40%, and 64.40%. Held-out language, target selection,
analysis, components, dictionary fidelity, alternative recovery, promotions, and every main
negative category remain unchanged. The current aggregate and diagnostic hashes are recorded
above.

A bounded follow-up extends the same retry only for exactly two-syllable pure-Hangul words when
the detector line is above 14.1 px and no taller than 15.9 px. A broader 17.7 px trial with the
same 94% confidence floor recovered one main context case but added six, so it was rejected. The
retained two-tier boundary removes only `dev-plain-0257|context|`, adds or changes no diagnostic,
and reduces the diagnostic count from 354 to 353. Aggregate functional context and first-popup
correctness rise to 91.70% and 86.80%. The 12 px context/popup stratum rises from 86.00% / 82.80%
to 86.40% / 83.20%, and natural punctuation rises from 87.20% / 82.80% to 87.60% / 83.20%.
Quick diagnostics remain byte-identical. Stress, held-out language, target selection, analysis,
components, dictionary fidelity, alternative recovery, promotions, and every negative category
remain unchanged. The current aggregate and diagnostic hashes are recorded above.

The following context batch recovers a word boundary before an opening straight or curly double
quote when a high-confidence Hangul prefix is fused to a multi-syllable Hangul quoted start and a
later high-confidence eojeol on the same detector line contains the matching closer. The closer may
have at most one attached Hangul syllable, and both opening and closing segments independently meet
the 95% confidence floor. This prevents a weak punctuation hallucination from triggering the split.

The exact full comparison removes only `dev-plain-0482|context|`, adds or changes no diagnostic,
and reduces the diagnostic count from 353 to 352. Whole-eojeol OCR, functional context, exact
sentence transcription, and first-popup correctness rise to 98.63%, 91.75%, 73.40%, and 86.85%.
The 14 px OCR/context/transcription/popup stratum rises from 97.04% / 84.40% / 57.60% / 80.00% to
97.11% / 84.80% / 58.00% / 80.40%; the ellipsis stratum rises from
98.66% / 92.40% / 54.00% / 87.20% to 98.73% / 92.80% / 54.40% / 87.60%. Quick diagnostics remain
byte-identical, while stress, held-out language, target selection, analysis, dictionary fidelity,
alternative recovery, promotions, and every negative category remain unchanged. The current
aggregate and diagnostic hashes are recorded above.

Privacy-safe review of the remaining 14 px failures isolated a line-initial two-syllable Hangul
substitution on a browser-rendered single line with ellipsis punctuation. The accepted OCR retry
is limited to the first segment on a 17.5-17.7 px detector line, an original confidence from 70%
through 80%, and narrow width, leading-margin, and following-gap ratios. It trims two pixels from
the left edge, checks five adjacent right boundaries after autocontrast, 2x bicubic scaling, and
binarization, and accepts only one unanimous same-length Hangul alternative whose weakest reading
is at least 99.8% confident.

The exact full comparison removes only `dev-plain-0994|context|`, adds or changes no diagnostic,
and reduces the full diagnostic count from 352 to 351. Whole-eojeol OCR, functional context, exact
sentence transcription, and first-popup correctness rise to 98.64%, 91.80%, 73.45%, and 86.90%.
The 14 px OCR/context/transcription/popup stratum rises from 97.11% / 84.80% / 58.00% / 80.40% to
97.14% / 85.20% / 58.40% / 80.80%; the ellipsis stratum rises from
98.73% / 92.80% / 54.40% / 87.60% to 98.76% / 93.20% / 54.80% / 88.00%. Quick diagnostics remain
byte-identical, while stress, held-out language, target selection, analysis, dictionary fidelity,
alternative recovery, promotions, and every negative category remain unchanged. The accepted
aggregate and privacy-safe diagnostic hashes are recorded above.

The next 24 px review accepts two independent OCR profiles. A line-initial three-syllable Hangul
substitution is retried only after two leading detector segments produce no Hangul and the retained
noise is a weak single digit. The detector height, word width, leading margin, following gap, and
98-99% original confidence are all bounded. Five adjacent right-edge crops must unanimously produce
the same different three-syllable Hangul reading after autocontrast and 2x scaling, each at 99.8%
confidence or better. The second profile joins a three-syllable and one-syllable Hangul pair only
when its 34.5-35.0% line-height gap is substantially smaller than both neighboring gaps, the glyph
pitches and segment confidences match the reviewed bounds, and raw plus enhanced combined crops
both reproduce the exact concatenation at 98.8% and 99.5% confidence respectively.

The exact full comparison removes only `dev-plain-1830|context|` and
`dev-plain-1838|context|`, adds or changes no diagnostic, and reduces the count from 351 to 349.
Aggregate functional context, exact sentence transcription, and first-popup correctness rise to
91.90%, 73.55%, and 87.00%. The 24 px OCR/context/transcription/popup stratum rises from
98.75% / 93.60% / 74.40% / 87.20% to 98.81% / 94.40% / 75.20% / 88.00%, clearing that size's
exceptional popup floor. Comma-colon context/popup rises to 95.60% / 90.40%, and terminal
context/popup rises to 92.00% / 84.80%. Quick diagnostics remain byte-identical; stress, held-out
...
A required-render follow-up recovers one crowded four-syllable Hangul substitution on a
17.5-17.7 px detector line. The profile bounds the selected confidence, word width, preceding gap,
and following overlap to the reviewed geometry. Autocontrasted crops thresholded independently at
208, 216, and 224 are recognized after 2x Lanczos resampling; all three must agree on one different
same-length pure-Hangul reading, and their weakest confidence must reach 91% and exceed the
selected reading.

The exact full comparison removes only `dev-plain-1154|context|`, adds or changes no diagnostic,
and reduces the total from 345 to 344. Aggregate functional context, exact sentence transcription,
and first-popup correctness rise to 92.20%, 73.85%, and 87.25%. The 14 px
OCR/context/transcription/popup stratum rises to 97.18% / 85.60% / 58.80% / 81.20%, and terminal
punctuation OCR/context/transcription/popup rises to 98.60% / 93.60% / 87.60% / 86.00%. Quick
diagnostics remain byte-identical; stress, held-out language, target selection, analysis,
...
A required-render 12 px follow-up recovers one internal four-syllable Hangul substitution on a
14.0-14.2 px detector line. The raw candidate must be 77-78% confident and 3.54-3.56 line heights
wide, overlap its preceding segment by 7-8% of line height, and leave a 21-22% following gap.
Six independently shifted crops are recognized after 3x or 4x bicubic resizing and 1.5x contrast.
All readings must agree on one same-length pure-Hangul candidate, preserve the first two and final
characters, differ at exactly one internal character, exceed the raw confidence, and have a
weakest confidence of at least 95%. The reviewed case reaches 95.64% weakest confidence.

The exact full v4.16 comparison removes only `dev-plain-1569|context|`, adds or changes no
diagnostic, and reduces the total from 341 to 340. Aggregate OCR/context/transcription/popup rises
to 98.68% / 92.40% / 74.05% / 87.45%. The 12 px OCR/context/transcription/popup stratum rises
from 98.37% / 87.20% / 64.00% / 83.60% to 98.40% / 87.60% / 64.40% / 84.00%.
Quick diagnostics remain byte-identical; target selection, stress, held-out language, analysis,
dictionary fidelity, alternative recovery, promotions, and every negative category are unchanged.
The accepted aggregate and privacy-safe diagnostic SHA-256 values are the current values above.

...
def test_position_visible_values_moves_target_and_omits_clipped_words() -> None:
    values = [
        {
            "index": index,
            "raw": "한국어",
            "core": "한국어",
            "box": [120.0, float(150 + index * 100), 180.0, float(180 + index * 100)],
            "core_box": [
                120.0,
                float(150 + index * 100),
                180.0,
    ...
    positioned, offset = _position_visible_values(values, 7)

    assert offset == -170.0
    assert [item["index"] for item in positioned] == [1, 2, 3, 4, 5, 6, 7]
    assert positioned[-1]["core_box"] == [120.0, 680.0, 180.0, 710.0]
...
def test_acquire_plain_cleans_partial_file_after_network_failure(
    ...
    with pytest.raises(OSError, match="network unavailable"):
        acquire_plain(tmp_path / "acquired", source_lock=source_lock, malgun=malgun, fetcher=fail)
    assert not (tmp_path / "acquired/data.conllu.part").exists()
...
def test_auxiliary_oracle_groups_both_helping_parts_of_speech_first() -> None:
    ...
    ordered = _ordered_oracle_entries(entries, "버리다", ("보조 동사", "보조 형용사"))
...
def test_render_matrix_balances_size_punctuation_renderer_and_font(tmp_path: Path) -> None:
    acquisition = {"system_fonts": {"malgun-gothic": {"path": str(tmp_path / "malgun.ttf")}}}
    specs = [
    ...
    assert {
        scale: sum(item.scale_percent == scale for item in specs) for scale in (100, 125, 150, 200)
...
def test_desktop_renderer_keeps_last_target_of_long_text_in_viewport(
    tmp_path: Path,
) -> None:
    font = Path("C:/Windows/Fonts/malgun.ttf")
    if not font.is_file():
        pytest.skip("Malgun Gothic is unavailable")
    words = ("한국어",) * 80 + ("마지막",)
    value = RenderInput(words, 80, "마지막")
    spec = RenderSpec(
        "desktop",
        "malgun-gothic",
        font,
    ...
    lines, target_box = DesktopRenderer().render(value, spec, tmp_path / "long.png")

    assert 0.0 <= target_box[1] < target_box[3] <= 720.0
    assert any(item["text"] == "마지막" for line in lines for item in line["eojeols"])
...
def test_browser_renderer_keeps_last_target_of_long_text_in_viewport(
    browser_renderer: BrowserRenderer, tmp_path: Path
) -> None:
    font = Path("C:/Windows/Fonts/malgun.ttf")
    if not font.is_file():
        pytest.skip("Malgun Gothic is unavailable")
    ...
    lines, target_box = browser_renderer.render(value, spec, tmp_path / "long-browser.png")

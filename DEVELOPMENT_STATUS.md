...
- Paddle-compatible ONNX CPU detector/recognizer with CTC-guided eojeol crops, conservative
  ...
  sentence context, exact reconstructed word geometry, and bounded recognition retries;
...
equivalent learner interpretations, eight corpus-oracle defects, and nine genuinely ambiguous
...
The 14 px OCR/context/transcription/popup stratum rises to 97.14% / 85.20% / 58.40% / 80.80%, and
the ellipsis stratum rises to 98.76% / 93.20% / 54.80% / 88.00%. Quick diagnostics remain
byte-identical; stress, held-out language, target, analysis, dictionary, promotion, and every
negative category are unchanged. The accepted aggregate and privacy-safe diagnostic SHA-256 values
are `945b53e23858252cdfc1f03fb469d26660b83e6d65215bc07112646eeb61c262` and
`c3e4e166357d01849a82897dcb7dbd38ff08a797f1fb7259053ee727ef8a0bdc`.

The next 24 px review accepted two independent OCR profiles. A line-initial three-syllable Hangul
...
language, target, analysis, dictionary, promotion, and every negative category are unchanged. The
accepted aggregate and privacy-safe diagnostic SHA-256 values are
...
The 16 px follow-up accepts two independently confirmed OCR profiles. A four-syllable Hangul
word following two non-Hangul detector fragments is expanded left only when the retained fragment
is a weak single Latin character and the detector height, word width, leading margin, following
gap, and original confidence match the reviewed band. Five left-edge crops after autocontrast and
2x bicubic scaling must unanimously add the same leading Hangul syllable at 99.9% confidence or
better. The second profile corrects an overlapping one-plus-three-syllable pair only within narrow
confidence, overlap, neighboring-gap, pitch, and union-width bands. Five adjacent right edges at
native and 2x scale supply ten readings; every reading must agree on the same four-syllable Hangul
word, retain the three-syllable suffix, and reach at least 99.6% confidence.
...
aggregate and privacy-safe diagnostic SHA-256 values are
...
14.0-14.2 px detector line. The profile requires 77-78% raw confidence, a width of 3.54-3.56 line
heights, a 7-8% overlap with the preceding segment, and a following gap of 21-22% of line height.
Six independently shifted crops are recognized after 3x or 4x bicubic resizing and 1.5x contrast.
All six readings must agree on one same-length pure-Hangul candidate, preserve the first two and
last characters, differ at exactly one internal character, reach 95% minimum confidence, and
exceed the raw reading. The reviewed case has 95.64% weakest consensus confidence.
